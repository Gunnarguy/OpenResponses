...
| Property             | Type     | Required | Description                            | App Status & Implementation Details |
| :------------------- | :------- | :------- | :------------------------------------- | :---------------------------------- |
| `type`               | `String` | **Yes**  | Must be `"input_audio"`.               | ❌ Not supported in this app. Audio input was removed. |
| `input_audio.data`   | `String` | **Yes**  | Base64-encoded audio data.             | ❌ Not supported. |
| `input_audio.format` | `String` | **Yes**  | The audio format (e.g., `mp3`, `wav`). | ❌ Not supported. |
...
| `input`        | `String` or `Array`  | **Yes**  | The core content for the model. Can be a simple string for user input or a rich array of `InputItem` objects for multimodal content.   | **Partially Implemented**. The `buildRequestObject` function handles both text strings and multimodal input arrays. `buildInputMessages` supports `input_text` and `input_image`, but audio and direct file uploads are not yet integrated. |
| `conversation` | `String` or `Object` | No       | The conversation this response belongs to. Can be a conversation ID string or a full conversation object. Manages state automatically. | **Partially Implemented**. The app maintains state by passing `previous_response_id`. It does not yet use the full conversation object or the Conversations API.                                                                            |
| `stream`       | `Bool`               | No       | If `true`, the server streams back Server-Sent Events (SSE) as the response is generated. Defaults to `false`.                         | **Partially Implemented**. The `streamChatRequest` function in `OpenAIService.swift` sets `"stream": true`, but does not yet handle all possible streaming events.                                                                          |
| `background`   | `Bool`               | No       | If `true`, the model response runs in the background. Defaults to `false`.                                                             | **Not Implemented**. The `Prompt` object does not have a `backgroundMode` property.                                                                                                                                                         |
| `tools`        | `Array`              | No       | A list of tool configurations the model can use, such as `web_search`, `code_interpreter`, etc.                                        | **Partially Implemented**. `buildRequestObject` includes some tools, but `computer` and other advanced tool configurations are missing.                                                                                                     |
| `tool_choice`  | `String` or `Object` | No       | Forces the model to use a specific tool.                                                                                               | **Not Implemented**. The `Prompt` object does not have a `toolChoice` property.                                                                                                                                                             |
| `include`      | `Array<String>`      | No       | Specifies additional data to include in the output (e.g., `message.output_text.logprobs`).                                             | **Not Implemented**. The `buildRequestObject` function does not construct the `include` array.                                                                                                                                              |
...
| Property             | Type     | Required | Description                            | App Status & Implementation Details |
| :------------------- | :------- | :------- | :------------------------------------- | :---------------------------------- |
| `type`               | `String` | **Yes**  | Must be `"input_audio"`.               | ❌ Removed. |
| `input_audio.data`   | `String` | **Yes**  | Base64-encoded audio data.             | ❌ Removed. |
| `input_audio.format` | `String` | **Yes**  | The audio format (e.g., `mp3`, `wav`). | ❌ Removed. |
