...
| **Advanced Tools**          | 🟡 **Mostly Complete** | Computer Use tool complete with proper API encoding (limited to compatible models); Custom Function tools complete; MCP partial. |
...
| **Computer Use Tool**                | ✅ **Complete with Enhanced Error Prevention**. Full API integration, UI controls, tool configuration, and streaming event handling implemented. Uses `computer_use_preview` API endpoint. **Enhanced**: Includes automatic preflight resolution system that prevents 400 "No tool output found for computer call" errors. **Note:** Hosted computer use is only supported via the `computer-use-preview` model; it is disabled for gpt-5, gpt-4.1 series, gpt-4o, and most other chat models. | - ✅ Computer tool configuration in `APICapabilities.swift` and `buildTools()`.<br>- ✅ Settings UI toggle and model compatibility checking.<br>- ✅ API parameter encoding and include options.<br>- ✅ Streaming event handling for computer screenshots and action confirmations.<br>- ✅ Fixed main thread issues in screen size detection.<br>- ✅ **NEW**: Automatic pending call resolution in `ChatViewModel.resolvePendingComputerCallsIfNeeded()`.<br>- ❌ **Limitation:** Only available on the `computer-use-preview` model per API. | 0.9        |
