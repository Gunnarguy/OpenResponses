...
| **Basic Tools**             | 🟡 **Partial**       | Web search, code interpreter, and file search are partially integrated.                         |
| **Advanced Tools**          | 🟡 **Partial**       | Custom tools, MCP, and calculator are implemented; `computer` use is missing.                   |
| **Streaming Response**      | 🟡 **Partial**       | Core event handling is in place, but many advanced events are not yet handled.                  |
...
| **Advanced Parameters**     | 🟡 **Partial**       | Some advanced model parameters are supported, but many are missing from the UI and requests.    |
...
| **Computer Use Tool**                | A built-in tool for remote browser and desktop tasks. Not yet implemented.                                                                                    | - Add `computer` tool configuration in `buildTools()`.<br>- Implement UI to display remote screen images and capture user confirmations.<br>- Integrate remote tool session management.                                                                | 0.7        |
| **gpt-image-1 & Streaming Previews** | The latest API adds `gpt-image-1` with multi-turn editing and streaming previews. The app currently uses DALL-E only.                                         | - Update tool configuration to use `image_generation` with `model: gpt-image-1`.<br>- Handle streaming events for image generation and update UI with partial previews.<br>- Support multi-turn editing by adding instructions to subsequent requests. | 0.9        |
