...
| **Computer Use Tool**                | ❌ **Not Started**. Comments in code show this was explicitly removed ("Computer Use (Preview) removed").                                                                 | - Add `computer` tool configuration in `buildTools()`.<br>- Implement UI to display remote screen images and capture user confirmations.<br>- Integrate remote tool session management.                                                                            | 0.7        |
...
| **File Search Enhancements**         | ✅ **Complete**. Multi-vector-store search is implemented. Users can input comma-separated vector store IDs.                                                              | - ✅ Multiple vector store IDs supported via comma-separated input.<br>- ✅ `createFileSearchToolConfiguration` handles array of IDs.<br>- ✅ UI accepts comma-separated vector store IDs.                                                                         | 0.6        |
