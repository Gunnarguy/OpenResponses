...
### Example: Hosting a Notion MCP server

The Responses API can only reach MCP servers that are exposed over HTTPS. To integrate Notion (or any other provider without an official connector), deploy the reference Notion MCP server and supply your Notion integration token when the server runs.

Deploy the Notion MCP server locally before hosting:
...
NOTION_API_KEY="secret_xxx" node dist/index.js
```

To host the server, containerize it or use a platform like Vercel, Fly.io, Railway, or Cloud Run:
...
Once deployed (for example at `https://your-notion-mcp.railway.app/sse`), point the Responses API request at that URL. Optionally limit the allowed MCP tools for additional control.
...
response = client.responses.create(
    ...
    tools=[{
        "type": "mcp",
        "server_label": "notion",
        "server_description": "Hosted Notion MCP workspace integration",
  "server_url": "https://your-notion-mcp.railway.app/sse",
        "authorization": "secret_your_notion_token",
        "require_approval": "never",
        "allowed_tools": ["search", "create_page", "update_page"],
    }],
    input="Search my Notion workspace for AI-related pages",
)
...
> **Tip:** Desktop-focused MCP runtimes (e.g., Claude Desktop) expose only local endpoints. When targeting the Responses API, always host the MCP server on the public Internet.
