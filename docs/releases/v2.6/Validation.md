...
GitHub CI selects an available iPhone simulator, executes `xcodebuild test -only-testing:OpenResponsesTests`, and retains its xcresult bundle. Runs 34261694919 and 34262928247 on the macOS 15 image (Xcode 26.3, iOS 26.2 simulator) failed with 29 crashes in `swift_task_deinitOnExecutor` when `@MainActor` classes were released from synchronous tests (swiftlang/swift#87316), not with test assertions. Commit `5b270d8` moved the workflow to the macOS 26 image with Xcode 26.6 (17F113) pinned; run 34267148194 then passed all 294 tests on the iPhone 17 Pro simulator, iOS 26.5. A local rerun with the same toolchain and runtime also passed 294 tests. A one-off matrix on the macOS 26 image (run 34267002652; its branch was deleted afterwards) reproduced the abort with Xcode 26.6 on the iOS 26.2 simulator (eight crash restarts in the same tests, run aborted) and not on iOS 26.4 or 26.5; Xcode 26.3 on iOS 26.5 passed 294 tests, so the defect sits in the iOS 26.2 simulator runtime rather than the compiler. The iOS 26.4 cell's only failure was `BrowserLiveSiteTests.testExampleAndIANAWithHistory` (WebKit `InvalidTransition`, live network), unrelated to the abort. Xcode Cloud has not run the test action.
...
- Isolated-deinit hand-off: the archive toolchain emits isolated deinits for the app's `@MainActor` classes (52 in the Xcode 26.6 Debug build) and delegates them to the OS runtime on iOS 18.4 or newer. That runtime path aborted on the iOS 26.2 simulator when such an object was released outside a Swift task while a task-local was bound on the thread; only XCTest has been observed to bind one, and the app binds none. Not measured on device runtimes between 18.4 and 26.3. Decide whether to add explicit `nonisolated deinit {}` declarations or accept the exposure.
